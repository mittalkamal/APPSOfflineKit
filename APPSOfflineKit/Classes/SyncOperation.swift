...
//  APPSOfflineKit
//
//  Created by Ken Grigsby on 2/20/16.
...
//        
//         self.log(.debug, msg: "Execute:")
//         self.log(.debug, msg: "'\(self.soupManager.soupDescription.soupName)'")
//       
//         soupManager.updateRemoteData { [weak self] (success) -> Void in
//                guard let strongSelf = self else { return }
//                
//                strongSelf.log(.debug, msg: "'\(strongSelf.soupManager.soupDescription.soupName)' updateRemoteData completed: \(success ? "success" : "failed")")
...
//    
    ...
    override open func execute() {
        ...
        if let soupId = soupId {
            soupManager.updateRemoteDataWithSoupId(soupId) { [weak self] (success) -> Void in
                ...
                strongSelf.log(.debug, msg: "'\(strongSelf.soupManager.soupDescription.soupName)' updateRemoteDataWithSoupId:\(soupId)  completed: \(success ? "success" : "failed")")
                let error = strongSelf.operationError(success: success)
                strongSelf.finishWithError(error)
            }
        }
    ...
    open func operationError(success: Bool) -> NSError? {
        ...
        if ignoreErrors {
            if let soupId = soupId {
                log(.debug, msg: "ignoring error for SoupId:\(soupId) soupName: '\(soupManager.soupDescription.soupName)'")
            }
            else {
                log(.debug, msg: "ignoring error for soupName: '\(soupManager.soupDescription.soupName)'")
            }
            return nil
        }

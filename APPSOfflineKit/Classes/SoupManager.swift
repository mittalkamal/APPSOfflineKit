...
//        describe("Licensing_Transaction__c")
    ...
    open func countLocalDataWithQuerySpec(_ querySpec: SFQuerySpec) -> Int {
        ...
        if let error = error {
            log(.error, msg: "Error retrieving count of '\(soupDescription.objectType)' data from SmartStore: \(error.localizedDescription)")
            return 0
        }
        return Int(count)
    }
    ...
    open func fetchLocalDataWithQuerySpec(_ querySpec: SFQuerySpec) -> [T] {
        ...
        do {
            ...
            log(.error, msg: "Error retrieving '\(soupDescription.objectType)' data from SmartStore: \(error.localizedDescription)")
    ...
    open func updateRemoteDataWithSoupId(_ soupId: SoupId, completion: @escaping SyncCompletionBlock) {
        ...
        guard let soupObject = fetchLocalDataBySoupId(soupId) else {
            log(.error, msg: "Couldn't find object with soupId: \(soupId) in soup \(soupDescription.soupName)")
            completion(false)
            return
        }
    ...
    open func deleteLocalData(_ dataToDelete: SoupObject) {
        if dataToDelete.isLocallyCreated {
            ...
            dataToDelete.updateSoup(forFieldName: kSyncTargetLocallyUpdated, fieldValue: true)
